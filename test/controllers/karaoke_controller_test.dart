...
void main() {
  group('KaraokeState', () {
    ...
    test('hasActiveSpan true only when end > start and start >= 0', () {
      expect(
        const KaraokeState(currentStart: 0, currentEnd: 1).hasActiveSpan,
        isTrue,
      );
      expect(
        const KaraokeState(currentStart: 5, currentEnd: 10).hasActiveSpan,
        isTrue,
      );
      expect(
        const KaraokeState(currentStart: -1, currentEnd: 5).hasActiveSpan,
        isFalse,
      );
      expect(
        const KaraokeState(currentStart: 5, currentEnd: 5).hasActiveSpan,
        isFalse,
      );
      expect(
        const KaraokeState(currentStart: 5, currentEnd: 4).hasActiveSpan,
        isFalse,
      );
    });
  ...
  group('KaraokeController', () {
    ...
    test('onTtsStart resets fallback mode on each new utterance', () {
      ...
      expect(
        container.read(karaokeControllerProvider).fallbackSentenceMode,
        isTrue,
      );
    ...
    test(
      'onTtsStop clears active span and isSpeaking but keeps visibility',
      () {
        controller.show();
        controller.onTtsStart('hello');
        controller.onProgress('hello', 0, 5, 'hello');
    ...
    test(
      'full lifecycle: start → progress → stop → start clears prior text',
      () {
