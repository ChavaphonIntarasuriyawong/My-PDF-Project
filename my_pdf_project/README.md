...
PDF reading progress tracker. Save PDFs by upload or link, track reading progress.
...
| PDF metadata | `syncfusion_flutter_pdf` |
...
| `cloud_firestore` | ^5.6.7 | Books + users docs. |
...
| `syncfusion_flutter_pdf` | ^27.1.48 | PDF metadata extraction at upload. |
...
```
lib/
├── core/          theme, errors, constants, network (proxy)
