...
| PDF metadata | `syncfusion_flutter_pdf` 27.1.48 |
...
| `syncfusion_flutter_pdf` | Metadata (`author`, `year`). | All |
...
| Text-to-speech (mobile + web) | PENDING — not yet implemented this sprint |
| Feature flag (Firebase Remote Config) | PENDING — one major feature must be gated before submission. Wire alongside next major feature (web reader / recents rail / TTS when added). |
