...
| Purpose | Save PDFs (link or upload), track reading progress |
...
| `cloud_firestore` | `users`, `books` collections. | All |
...
| `syncfusion_flutter_pdf` | Metadata (`author`, `year`) extraction at upload. | All |
...
│   │   ├── domain/  book_model
│   │   └── presentation/  home_screen, new_book_screen, book_info_screen, library_controller, library_providers
│   ├── reader/
│   │   └── presentation/  reading_screen
│   └── profile/
...
books/{bookId}  { title, link, totalPages, currentPage, progress, status, ownerId, lastReadAt(ISO), author?, year? }
```
...
### Cascades
- Delete book → Supabase object purged + local cache/thumb purged.
...
Removed (no longer in code): bookshelves, notes, TTS, edit-profile, recently-opened rail, local Hive storage.
