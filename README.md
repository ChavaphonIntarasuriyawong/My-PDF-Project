...
- **Karaoke captions** — slide-up TTS pane with word-sync highlighting, click-to-seek, and live speed slider (gated by Firebase Remote Config kill-switch `karaoke_tts_enabled`)
- **Streak tracker** — consecutive-day reading streak with confetti at 7 / 30 / 100 day milestones
- **Book-finish celebration** — one-shot confetti + snackbar the first time you reach the last page
- **Surprise Me** — home FAB picks a random non-finished, non-recent book to read
- **Achievements** — 7-badge catalog (First Steps, Bookworm, Streak Starter / On Fire / Inferno, Surprise Reader, Karaoke Star) surfaced in the profile screen
- Profile (name + email)
...
| Feature flags | Firebase Remote Config (karaoke kill-switch) |
| File storage | Supabase Storage (bucket `pdfs`) |
| CORS proxy | Supabase Edge Function `pdf-proxy` |
| Local storage | Hive (recents, streak, finish flags, achievement counters) |
...
| `firebase_remote_config` | ^5.4.0 | Server-side kill-switch for karaoke captions (`karaoke_tts_enabled`). |
...
| `hive` + `hive_flutter` | ^2.2.3 / ^1.1.0 | Local `app_prefs` box → recents, streak, finish flags, achievement counters. |
| `confetti` | ^0.7.0 | One-shot confetti bursts for book finish + streak milestones. |
...
```
lib/
├── core/          theme, errors, constants, config (Remote Config flags), local (Hive: recents/streak/finish/achievements), network (CORS proxy)
├── features/      auth, library, reader, profile  (data/domain/presentation per feature)
│                  └── reader/presentation/{controllers,widgets}/  karaoke caption pane + state
...
- `lib/main.dart` — Supabase URL + anon key, Hive box open, Firebase + Crashlytics + Remote Config init
...
- `lib/core/config/feature_flags.dart` — Remote Config flag providers (`karaokeEnabledProvider`)
...
| Key | Default | Effect |
|---|---|---|
| `karaoke_tts_enabled` | `true` | Show / hide the karaoke captions pane in the reader. Flip to `false` in the Firebase console for an instant kill-switch — no redeploy needed. |
